...
class Scheduler<T> {
    ...
    public void add(Date d, T t){
        this.schedules.put(d, t);
    }

    public boolean remove(Date d){
        if(this.schedules.containsKey(d)){
            this.schedules.remove(d);
            return true;
        }
        return false;
    }
    ...
    public ArrayList<T> getAll(Date begin, Date end){
        ...
        schedules.keySet().stream().filter(schedule -> schedule.after(begin) && schedule.before(end)).forEach(schedule -> sortedList.add(schedules.get(schedule)));
        return sortedList;
    }

    public T getFirst(){
        return this.schedules.firstEntry().getValue();
    }

    public T getLast(){
        return this.schedules.lastEntry().getValue();
    }
}
